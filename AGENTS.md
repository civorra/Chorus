...
| `chorus-feed <sandbox-name> <corpus>` | command | `./agent/skills/chorus-feed.md` — enriches sandbox knowledge: KB org per agent + YAML (Mode A init / Mode B incremental enrichment). Phase 1.0: pre-annotation scan (`CHORUS:no_auto_rules` markers) excludes doctrinal sections (15–25% of large corpora) from analysis before Phase 1.1. Pre-split mode (when agent files from `chorus-corpus-scoping` Phase 2 exist): runs Mode A once per agent file, enforces `no_auto_rules` on CONTEXT/SHARED sections — virtually eliminates structural `⏭ Deferred`. Phase 6.5: coverage report with mandatory too-ambiguous retry (Step 2b), CORPUS: header audit (Step 1b), stability point check (Step 4b) → `chorus-review-kb` suggested only at SP1/SP2. Convergence loop: SP1/SP2 → review-kb → strengthen → stress → enrich → repeat until Deferred=0 + CORPUS:unresolved=0 + Uncovered=0 + Orphan=0 + strengthen gaps=0. Loads `chorus-corpus-directives.md` only when ≥2 independently-versioned normative sources; auto-loads `chorus-corpus-scoping.md` on corpora >2 files or >~50 pages. | `architect` |
...
| `chorus-corpus-scoping <sandbox-name> [--corpus <file(s)>] [--split] [--rescan]` | command | `./agent/skills/chorus-corpus-scoping.md` — domain-agnostic pre-`chorus-feed` structural scoping gate: proposes agents, Frames, inter-Frame relationships, control slots (`_DEFAULT`/`_NEEDED`/`_AFTER`/`_BEFORE`/`_REQUIRE`), BOARD slots, and pipeline order → `SCOPING.md`, held for operator confirmation before any KB/YAML/Helpers.pm is generated. **Phase 2 (auto after CONFIRMED or via `--split`):** multi-target corpus pre-split — extracts one focused corpus file per agent (`corpus/NNN-<slug>.md`), each section tagged `PRIMARY`/`SHARED`/`CONTEXT`. Classification uses (1) inline `CHORUS:no_auto_rules` markers, (2) `CORPUS-DIRECTIVES.md` `no_auto_rules` declarations (STEP 0, takes precedence over keyword scoring), (3) keyword + identifier scoring against agent intents. Deterministic section ordering: primary key = source file number, secondary = position within file. Handles partial pre-split resumption automatically. `--split`: force Phase 2 regeneration. `--rescan`: force Phase 1 re-analysis (resets to DRAFT, deletes agent files). | `architect` |
