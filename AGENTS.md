...
| `chorus-check <sandbox-name> <project-file>` | command | `./eca/skills/chorus-check.md` — generates Feed+Agent+Expert+run.pl from the KB, runs the pipeline, produces the compliance report | `architect` |
| `chorus-create-project <sandbox-name> <file.json>` | command | `./eca/skills/chorus-create-project.md` — creates a JSON project file from the org KB (slots, thresholds, conforming/KO variants) — ⛔ never reads Helpers.pm or Feed.pm | `architect` |
