...
| `deep-chorus [<focus>]` | command | `./agent/skills/deep-chorus.md` — deep implementation analysis of `Chorus::*` sources; optional focus: `backward-chaining` · `explain` · `planner` | `architect` |
...
| `chorus-import-project <sandbox-name> <source…> [--out <f.json>] [--batch]` | command | `./agent/skills/chorus-import-project.md` — aligns the terminology of a project document (PDF/Word/Excel/inline) with KB slots. **3 modes:** unit (1 file), fusion (N files → 1 JSON), batch (directory/glob → 1 JSON per file + synthesis report) | `code` |
